    ...
    <section className="py-24 bg-[#F9F8F6]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <h2 className="font-serif text-3xl md:text-4xl text-stone-900 mb-4">{t('testimonials.title')}</h2>
          <p className="text-stone-500">{t('testimonials.subtitle')}</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {reviews.map((review) => (
            <div key={review.id} className="bg-white p-8 shadow-sm hover:shadow-md transition-shadow border border-stone-100">
              ...
              <p className="text-stone-700 mb-6 italic leading-relaxed">"{t(review.textKey)}"</p>
              <div>
                <p className="font-bold text-stone-900 text-sm">{t(review.nameKey)}</p>

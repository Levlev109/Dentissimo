  ...
  return (
    <div className="group relative">
      ...
      <div className="text-center">
        ...
        <h3 className="font-serif text-lg text-stone-900 mb-1 group-hover:text-[#D4AF37] transition-colors cursor-pointer">
          {product.name}
        </h3>
        {product.description && (
          <p className="text-xs text-stone-500 mb-2 line-clamp-2">{product.description}</p>
        )}
        <p className="text-stone-600 font-medium">€{product.price.toFixed(2)}</p>
      </div>
    </div>
  );
};

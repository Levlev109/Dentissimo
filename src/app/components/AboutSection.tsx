...
export const AboutSection = () => {
  ...
  return (
    <section id="about" className="bg-[#F0F7FB] text-stone-900 overflow-visible">
      ...
      <div className="py-20 bg-white/60 border-y border-sky-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {regera.map((item, index) => {
              ...
              return (
                <motion.div
                  key={index}
                  className="bg-sky-50 rounded-xl p-8 border border-sky-100 text-center hover:border-sky-300 transition-colors duration-500"
                  initial={{ opacity: 0, y: 20 }}
                  whileInView={{ opacity: 1, y: 0 }}
                  viewport={{ once: true }}
      ...
      <div className="py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {/* Certifications row */}
          <motion.div
            className="mt-16 pt-12 border-t border-sky-200/50"
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.6 }}
          >
            <div className="grid grid-cols-3 sm:grid-cols-5 gap-4 sm:gap-6 md:gap-12 mb-6">
              {certifications.map((cert) => (
                ...
                >
                  <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-full border border-stone-200 flex items-center justify-center mx-auto mb-2 group-hover:border-sky-400 group-hover:bg-sky-50 transition-all duration-300">
                    <cert.icon size={22} className="text-stone-400 group-hover:text-sky-600 transition-colors" />
                  </div>
                  <span className="text-[10px] sm:text-xs text-stone-500 font-medium tracking-wide group-hover:text-stone-700 transition-colors leading-tight block">{t(cert.titleKey)}</span>
                </div>
              ))}
            </div>
            ...
            <AnimatePresence mode="wait">
              {activeCert && (() => {
                ...
                return (
                  ...
                  >
                    <div className="bg-sky-50 border border-sky-200 rounded-xl p-5 sm:p-6 max-w-lg mx-auto relative mb-4">
                      <button
                        onClick={(e) => { e.stopPropagation(); setActiveCert(null); }}
                        className="absolute top-3 right-3 text-stone-400 hover:text-stone-700 transition-colors p-1"
                      >
                        <X size={16} />
                      </button>
                      <h5 className="text-stone-900 font-bold text-sm mb-2">{t(cert.titleKey)}</h5>
                      <p className="text-stone-600 text-sm leading-relaxed">{t(cert.descKey)}</p>
                    </div>
                  </motion.div>
                );
              })()}

...
const MERCHANT_SECRET  = 'fdeabef2a62650cdf13ff3751c494debb56b3c33';
...
// ── HMAC-MD5 implementation (required by WayForPay) ──────────────────
...
export function openPayment(params: PaymentParams): Promise<'approved' | 'declined' | 'pending'> {
  ...
  // Signature string per WayForPay spec
  const signString = [
    MERCHANT_ACCOUNT,
    MERCHANT_DOMAIN,
    params.orderId,
  ...
  const merchantSignature = hmacMD5(MERCHANT_SECRET, signString);

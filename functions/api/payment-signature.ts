// Cloudflare Pages Function — server-side WayForPay signature generation
// The merchant secret NEVER reaches the browser
...
function bitRol(n: number, c: number) {
  return (n << c) | (n >>> (32 - c));
}
...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  const CORS_HEADERS = {
    'Access-Control-Allow-Origin': 'https://dentissimo.sale',
  ...
  try {
    ...
    if (!body.orderId || !body.orderDate || !body.amount ||
        !Array.isArray(body.productNames) || body.productNames.length === 0) {
    ...
    const secret = context.env.WAYFORPAY_SECRET;
    if (!secret) {
    ...
    const signString = [
      ...
      body.productCounts.join(';'),
      body.productPrices.join(';'),
    ].join(';');

    const signature = hmacMD5(secret, signString);

    return new Response(JSON.stringify({ merchantSignature: signature }), {
      status: 200,
      headers: { 'Content-Type': 'application/json', ...CORS_HEADERS },
    });
  } catch {
